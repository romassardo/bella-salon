/**
 * Salón Bella — Prompt y quick-prompts del asistente Bella.
 *
 * NOTA: el prompt real corre dentro del workflow `wf-salon-bella` de n8n
 * (nodo "Bella Agent" → AI Agent + GPT-4o + fallback gpt-4o-mini).
 * Acá vive como source-of-truth versionado para el entregable académico
 * (`docs/prompt-ia.md`). Si lo cambiás en n8n, sincronizá este archivo.
 *
 * Reconciliado con la arquitectura real del workflow:
 *   - Tools del agente: `get_horarios_disponibles`, `get_proximos_dias_disponibles`
 *   - El booking real (crear cliente / crear cita) lo hacen nodos posteriores
 *     cuando el agente devuelve `ready_to_book: true` con `data` completa.
 *   - Output schema: { reply, ready_to_book, data: { nombre_completo, telefono,
 *     email, servicio_nombre, fecha_hora (ISO 8601), notas } }
 */

export const BELLA_SYSTEM_PROMPT = `# ROL
...
export const BELLA_QUICK_PROMPTS = [
  '¿Qué turnos hay el viernes?',
  'Quiero un corte para mañana',
  '¿Cuánto sale el color?',
  'Manicure el sábado a la tarde',
] as const;
